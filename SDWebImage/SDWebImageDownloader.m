...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
        ...
        if (width + height > 0 && totalSize < expectedSize)
        {
            ...
            if (partialImageRef)
            {
                UIImage *image = SDScaledImageForPath1(url.absoluteString, [UIImage imageWithCGImage:partialImageRef]);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    ...
    {
        UIImage *image = SDScaledImageForPath1(url.absoluteString, imageData);

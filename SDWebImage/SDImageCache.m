...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    UIImage *image = SDScaledImageForPath1(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        image = SDScaledImageForPath1(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
        if (image)
        {
            [memCache setObject:image forKey:key];
        }

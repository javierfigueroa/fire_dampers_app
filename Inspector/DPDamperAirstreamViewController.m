//
//  DPDamperAirstreamViewController.m
//  Inspector
//
...
@interface DPDamperAirstreamViewController ()

@end

@implementation DPDamperAirstreamViewController
...
- (void)viewDidLoad
{
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"DamperAirstream" ofType:@"plist"];
...
{
    return [[self.damperAirstreams allKeys] count];
...
{ 
    ...
    NSNumber *status = [[self.damperAirstreams allKeys] objectAtIndex:[indexPath row]];
    NSDictionary *statusValues = [self.damperAirstreams objectForKey:[NSString stringWithFormat:@"%@", status]];
...
{    
    NSNumber *type = [[self.damperAirstreams allKeys] objectAtIndex:[indexPath row]];
    self.checkedType = type;
    if (delegate && [delegate respondsToSelector:@selector(didSelectDamperType:)]) {
        [delegate didSelectDamperAirstream:type];
    }
